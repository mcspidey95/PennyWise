...
namespace PennyWise.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<Transaction> SelectedTransaction = await _context.Transactions.Include(x => x.Category).Where(y => y.Date >= StartDate && y.Date <= EndDate).ToListAsync();
            ...
            //Doughunt Chart
            ViewBag.DoughnutChartData = SelectedTransaction.Where(i => i.Category.Type == "Expense").GroupBy(j => j.Category.CategoryId).Select(k => new
            {
            ...
            //Spline Chart
            List<SplineChartData> IncomeSummary = SelectedTransaction.Where(i => i.Category.Type == "Income").GroupBy(j => j.Date).Select(k => new SplineChartData()
            {
                day = k.First().Date.ToString("dd-MMM"),
                income = k.Sum(l => l.Amount),
            }).ToList();

            List<SplineChartData> ExpenseSummary = SelectedTransaction.Where(i => i.Category.Type == "Expense").GroupBy(j => j.Date).Select(k => new SplineChartData()
            {
                day = k.First().Date.ToString("dd-MMM"),
                expense = k.Sum(l => l.Amount),
            }).ToList();

            string[] last31Days = Enumerable.Range(0,31).Select(i => StartDate.AddDays(i).ToString("dd-MMM")).ToArray();
